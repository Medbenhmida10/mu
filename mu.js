(function () {
 ...
 /*--------------------------------------------------------------------------------------------------------------- */
        ...
        div0.innerHTML = '<script  id="oView' + widgetName + '" name="oView' + widgetName + '" type="sapui5/xmlview"><mvc:View controllerName="myView.Template" xmlns:core="sap.ui.core" xmlns:mvc="sap.ui.core.mvc" xmlns="sap.m"><Toolbar><Label width="auto" text="Search   " design="Bold"/><Input width="90%" placeholder="Type To Search" showClearIcon="true" value="{search>/query}" liveChange="onLiveChange" /></Toolbar> <Tree   id="Tree" items="{' + widgetName + '>/}" mode="MultiSelect"  selectionChange="onSelect"  includeItemInSelection="true" updateFinished="onDefaultSelction" ><StandardTreeItem title="{' + widgetName + '>text}" selected="{selected}"/></Tree></mvc:View></script>'
